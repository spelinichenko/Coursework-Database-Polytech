...
{
    public partial class UpdateAssessmentByLector : Form
    {
        SqlConnection conn;
        int id;
        public UpdateAssessmentByLector(int id, SqlConnection conn)
        {
            InitializeComponent();
            this.id = id;
            this.conn = conn;
        ...
        private void fillComboBox1()
        {
            string query = "SELECT DISTINCT email FROM students JOIN assessments " +
                ...
                "ON assessments.lector_group_subject_id = lectors_groups_subjects.lector_group_subject_id " +
                "WHERE lector_id=" + id;
        ...
        {
            string query = "SELECT DISTINCT email FROM students JOIN groups" +
        ...
        private void UpdateAssessmentByLector_Load(object sender, EventArgs e)
        {
            fillComboBox1();
            fillComboBox7();
        }
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            "ON assessments.student_id = students.student_id " +
                "WHERE students.email='" + comboBox1.Text.ToString() + "'" + " AND lectors.lector_id=" + id, conn);
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            SqlCommand sql = new SqlCommand("SELECT DISTINCT subject_name FROM assessments JOIN lectors_groups_subjects " +
                ...
                "WHERE students.email='" + comboBox1.Text.ToString() + "'" +
                "AND lectors.email ='" + comboBox2.Text.ToString() + "'" + " AND lectors.lector_id=" + id, conn);
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sql);
            DataTable dataTable = new DataTable();
            sqlDataAdapter.Fill(dataTable);
        ...
        {
            SqlCommand sql = new SqlCommand("SELECT DISTINCT lectors.email FROM lectors JOIN lectors_groups_subjects " +
                "ON lectors.lector_id = lectors_groups_subjects.lector_id JOIN groups " +
                "ON lectors_groups_subjects.group_id = groups.group_id " +
                "WHERE lectors_groups_subjects.lector_id =" + id + " AND lectors_groups_subjects.group_id = " +
                "(SELECT group_id FROM students WHERE students.email='" + comboBox7.Text.ToString() + "'" + ")", conn);
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sql);
            DataTable dataTable = new DataTable();
            sqlDataAdapter.Fill(dataTable);
