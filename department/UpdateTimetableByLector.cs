...
{
    public partial class UpdateTimetableByLector : Form
    {
        SqlConnection conn;
        int id;
        public UpdateTimetableByLector(int id, SqlConnection conn)
        {
            InitializeComponent();
            this.conn = conn;
            this.id = id;
        ...
        private void fillComboBox1()
        {
            string query = "SELECT DISTINCT groups.group_number FROM groups JOIN lectors_groups_subjects " +
                ...
                "ON lectors_groups_subjects.lector_group_subject_id = timetables.lector_group_subject_id JOIN lectors" +
                " ON lectors_groups_subjects.lector_id=lectors.lector_id" +
                " WHERE lectors.lector_id=" + id;
        ...
        {
            string query = "SELECT DISTINCT groups.group_number FROM groups JOIN lectors_groups_subjects " +
                "ON groups.group_id = lectors_groups_subjects.group_id JOIN lectors " +
                "ON lectors_groups_subjects.lector_id = lectors.lector_id " +
                "WHERE lectors.lector_id=" + id;
            SqlCommand sqlCommand = new SqlCommand(query, conn);
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sqlCommand);
        ...
        private void UpdateTimetableByLector_Load(object sender, EventArgs e)
        {
            fillComboBox1();
            fillComboBox9();
            fillComboBox12();
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            SqlCommand sqlCommand = new SqlCommand("SELECT DISTINCT email FROM lectors_groups_subjects JOIN lectors " +
                ...
                "ON lectors_groups_subjects.group_id = groups.group_id WHERE group_number='" + comboBox1.Text.ToString() + "'" + " AND lectors.lector_id=" + id, conn);
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sqlCommand);
            DataTable dataTable = new DataTable();
            sqlDataAdapter.Fill(dataTable);
        ...
        {
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter("exec timetable_on_week_with_id @group_number='" + comboBox12.Text.ToString() + "'", conn);
            DataTable dataTable = new DataTable();
            sqlDataAdapter.Fill(dataTable);
            dataGridView1.DataSource = dataTable;
        }
        ...
        private void comboBox9_SelectedIndexChanged(object sender, EventArgs e)
        {
            SqlCommand sqlCommand = new SqlCommand("SELECT DISTINCT email FROM lectors_groups_subjects JOIN lectors " +
                "ON lectors_groups_subjects.lector_id = lectors.lector_id " +
                " JOIN groups " +
                "ON lectors_groups_subjects.group_id = groups.group_id WHERE group_number='" + comboBox9.Text.ToString() + "'" + " AND lectors.lector_id=" + id, conn);
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sqlCommand);
            DataTable dataTable = new DataTable();
            sqlDataAdapter.Fill(dataTable);
