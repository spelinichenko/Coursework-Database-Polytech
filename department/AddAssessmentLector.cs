...
{
    public partial class AddAssessmentLector : Form
    {
        SqlConnection conn;
        int id;
        public AddAssessmentLector(int id, SqlConnection conn)
        {
            InitializeComponent();
            this.conn = conn;
            this.id = id;
        ...
        private void AddAssessmentLector_Load(object sender, EventArgs e)
        {
            fillComboBox1();
        }
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            SqlCommand sqlCommand = new SqlCommand("SELECT DISTINCT lectors.email FROM lectors JOIN lectors_groups_subjects " +
                ...
                "ON groups.group_id = students.group_id WHERE students.email='" + comboBox1.Text.ToString() + "'" + ")" + " AND lectors.lector_id=" + id, conn);
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sqlCommand);
            DataTable dataTable = new DataTable();
            sqlDataAdapter.Fill(dataTable);
