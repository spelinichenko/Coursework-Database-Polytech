...
{
    public partial class AddTimetableByLector : Form
    {
        int id;
        SqlConnection conn;
        public AddTimetableByLector(int id, SqlConnection conn)
        {
            InitializeComponent();
            this.id = id;
            this.conn = conn;
        ...
        private void fillComboBox2()
        {
            string query = "SELECT DISTINCT group_number FROM groups JOIN lectors_groups_subjects " +
                "ON groups.group_id = lectors_groups_subjects.group_id WHERE lector_id=" + id;
        ...
        private void AddTimetableByLector_Load(object sender, EventArgs e)
        {
            fillComboBox2();
        }
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            SqlCommand sqlCommand = new SqlCommand("SELECT DISTINCT lectors.email FROM lectors JOIN lectors_groups_subjects " +
                ...
                "WHERE groups.group_number ='" + comboBox2.Text.ToString() + "'" + " AND lectors.lector_id=" + id, conn);
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sqlCommand);
            DataTable dataTable = new DataTable();
            sqlDataAdapter.Fill(dataTable);
        ...
        {
            if (comboBox2.SelectedIndex == -1 && comboBox2.Text.Length == 0)
            {
